...
---- 
(2) MapPartitionsRDD[3] at mapValues at SCSuite.scala:20 []
 |  ShuffledRDD[2] at groupByKey at SCSuite.scala:20 []
 +-(2) MapPartitionsRDD[1] at map at SCSuite.scala:18 []
    |  ParallelCollectionRDD[0] at parallelize at SCSuite.scala:17 []
----
...
DataFrame je konstrukt Spark API, ktery reprezentuje tabulku slozenou z radku a sloupcu. Tato tabulka se muze rozkladat na desitkach pocitacu v ramci clusteru. Duvod ulozeni dat na vice pocitacu je zrejmy, bud jsou data prilis velka aby je bylo mozne ulozit na jediny pocitac, nebo je k jejich zpracovani potrebny vetsi vykon. Stejny koncept, ackoliv omezeny na jediny pocitac, pouzivaji API jako Python Pandas nebo R DataFrames. Toto usadnuje pouzivani Sparku se znalostmi techto API, napriklad jako doplmnujici nastroj pro praci s velkymi objemy dat.
Partitions::
