...
V nasledujici jednoduchem priklade spocitame kolik je lichych a sudych cisel v intervalu <1, 9>:
...
Pro tento priklad vyhodnoti Spark nasledujici retezec zpracovani:
...
----
<1> Na radku 1 nejprvnoze vytvorime zdrojovy dataset. Ten v tomto pripade inicializujeme primo v kodu ackoliv typickym pouzitim je nacist zdrojova data z persistetniho uloziste. 

<2> Na radku 2. pouzijeme RDD metodu *map*, tato metoda ma jako argument funkci, ktera trasnsformuje jednu z hodnot zdrojoveho RDD. V retezci zpracovani je jako vysledek tohotyo kroku MapPartitionsRDD, to znamena, ze se transformace provede  lokalne na urovni jednotlivych partition. Jako vysledek transformace je RDD dvojic kde je prvni hodnota zbytek po celociselnem deleni dvema. POkud jsou v RDD dvojice Spark automaticky bere prvni hodnotu jako klic.  

<3> Na radku 3. pomoci RDD metody groupByKey seskupime hodnoty se stejnym klicem. Vysledkem tedy bude RDD dvojice s prislusnym klicem kde jako hodnota bude kolekce vsechn hodnot odpovidajici, ktere maji ve zdrojovem RDD dany klic. V retezci zpracovani je jako vysledek tohotyo kroku ShuffledRDD, to znamena, ze
je treba provest preskladani v ramci vypocetniho clustru a seskupit hodnoty se stejnym klicem.

<4> Na radku 3. pomoci RDD mapValues transformujeme pouze hodnoty, klic ve dojici zustava nezmenen. Zde je jako hodnota kolekce cisel odpovidajici danemu zbytku po celociselnem deleni. V teto operaci tuto kolekci prevdeme na celel cislo odpovidajici poctu clenu teto kolekce. V retezci zpracovani je jako vysledek tohotyo kroku MapPartitionsRDD, transformace se take provede  lokalne na urovni jednotlivych partition. 

Vysledkem je 5 lichych hodnot (klic 1) a 4 sude hodnoty (klic 0):  
