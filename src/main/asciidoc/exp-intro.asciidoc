...
Počet iterací by se měl nastavit dostatečně vysoký aby nadále nedocházelo ke snížení výsledného RMSE, tj. tato hodnota by měla uvíznout ve svém minimu. Z <<hp-maxIter>> je zřejmé že výchozí počet iterací 10 je dostatečný, hodnota RMSE se ustálí po šesté iteraci.
